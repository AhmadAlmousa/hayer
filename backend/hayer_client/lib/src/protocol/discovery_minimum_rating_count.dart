/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: implementation_imports
// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: non_constant_identifier_names
// ignore_for_file: public_member_api_docs
// ignore_for_file: type_literal_in_constant_pattern
// ignore_for_file: use_super_parameters
// ignore_for_file: invalid_use_of_internal_member

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'package:serverpod_client/serverpod_client.dart' as _isc;

abstract class DiscoveryMinimumRatingCount
    implements _isc.SerializableModel, _isc.ProtocolSerialization {
  DiscoveryMinimumRatingCount._({
    required this.minimumRating,
    required this.count,
  });

  factory DiscoveryMinimumRatingCount({
    required double minimumRating,
    required int count,
  }) = _DiscoveryMinimumRatingCountImpl;

  factory DiscoveryMinimumRatingCount.fromJson(
    Map<String, dynamic> jsonSerialization,
  ) {
    return DiscoveryMinimumRatingCount(
      minimumRating: (jsonSerialization['minimumRating'] as num).toDouble(),
      count: jsonSerialization['count'] as int,
    );
  }

  double minimumRating;

  int count;

  /// Returns a shallow copy of this [DiscoveryMinimumRatingCount]
  /// with some or all fields replaced by the given arguments.
  @_isc.useResult
  DiscoveryMinimumRatingCount copyWith({
    double? minimumRating,
    int? count,
  });
  @override
  Map<String, dynamic> toJson() {
    return {
      '__className__': 'DiscoveryMinimumRatingCount',
      'minimumRating': minimumRating,
      'count': count,
    };
  }

  @override
  Map<String, dynamic> toJsonForProtocol() {
    return {
      '__className__': 'DiscoveryMinimumRatingCount',
      'minimumRating': minimumRating,
      'count': count,
    };
  }

  @override
  String toString() {
    return _isc.SerializationManager.encode(this);
  }
}

class _DiscoveryMinimumRatingCountImpl extends DiscoveryMinimumRatingCount {
  _DiscoveryMinimumRatingCountImpl({
    required double minimumRating,
    required int count,
  }) : super._(
         minimumRating: minimumRating,
         count: count,
       );

  /// Returns a shallow copy of this [DiscoveryMinimumRatingCount]
  /// with some or all fields replaced by the given arguments.
  @_isc.useResult
  @override
  DiscoveryMinimumRatingCount copyWith({
    double? minimumRating,
    int? count,
  }) {
    return DiscoveryMinimumRatingCount(
      minimumRating: minimumRating ?? this.minimumRating,
      count: count ?? this.count,
    );
  }
}
