/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: implementation_imports
// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: non_constant_identifier_names
// ignore_for_file: public_member_api_docs
// ignore_for_file: type_literal_in_constant_pattern
// ignore_for_file: use_super_parameters
// ignore_for_file: invalid_use_of_internal_member

// ignore_for_file: no_leading_underscores_for_library_prefixes

import 'package:serverpod_client/serverpod_client.dart' as _i1;

abstract class AdminCatalogHeatCell implements _i1.SerializableModel {
  AdminCatalogHeatCell._({
    required this.latitude,
    required this.longitude,
    required this.count,
  });

  factory AdminCatalogHeatCell({
    required double latitude,
    required double longitude,
    required int count,
  }) = _AdminCatalogHeatCellImpl;

  factory AdminCatalogHeatCell.fromJson(
    Map<String, dynamic> jsonSerialization,
  ) {
    return AdminCatalogHeatCell(
      latitude: (jsonSerialization['latitude'] as num).toDouble(),
      longitude: (jsonSerialization['longitude'] as num).toDouble(),
      count: jsonSerialization['count'] as int,
    );
  }

  double latitude;

  double longitude;

  int count;

  /// Returns a shallow copy of this [AdminCatalogHeatCell]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  AdminCatalogHeatCell copyWith({
    double? latitude,
    double? longitude,
    int? count,
  });
  @override
  Map<String, dynamic> toJson() {
    return {
      '__className__': 'AdminCatalogHeatCell',
      'latitude': latitude,
      'longitude': longitude,
      'count': count,
    };
  }

  @override
  String toString() {
    return _i1.SerializationManager.encode(this);
  }
}

class _AdminCatalogHeatCellImpl extends AdminCatalogHeatCell {
  _AdminCatalogHeatCellImpl({
    required double latitude,
    required double longitude,
    required int count,
  }) : super._(
         latitude: latitude,
         longitude: longitude,
         count: count,
       );

  /// Returns a shallow copy of this [AdminCatalogHeatCell]
  /// with some or all fields replaced by the given arguments.
  @_i1.useResult
  @override
  AdminCatalogHeatCell copyWith({
    double? latitude,
    double? longitude,
    int? count,
  }) {
    return AdminCatalogHeatCell(
      latitude: latitude ?? this.latitude,
      longitude: longitude ?? this.longitude,
      count: count ?? this.count,
    );
  }
}
