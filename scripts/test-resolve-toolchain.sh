...
mkdir -p "$(dirname "$flutter_bin")" "$(dirname "$path_dart")" \
  "$(dirname "$explicit_dart")"

for executable in "$flutter_bin" "$flutter_dart" "$path_dart" "$explicit_dart"; do
...
assert_equal "$flutter_dart" "$resolved" "Flutter's Dart should precede PATH"
